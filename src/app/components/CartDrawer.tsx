...
export default function CartDrawer() {
  ...
  // If cart gets emptied while on the form step, go back to cart step
  useEffect(() => {
  ...
  const resetDrawer = () => {
    closeCart();
    // Allow animation to finish before resetting state
    setTimeout(() => {
      setStep('cart');
    ...
    <>
      ...
      <div className={`cart-drawer ${isCartOpen ? 'open' : ''}`}>
        <div className="cart-drawer-header">
          <h2>{step === 'form' ? 'Détails' : 'Mon Panier'}</h2>
        ...
        <div className="cart-drawer-content">
          {step === 'success' ? (
            ...
            cart.length === 0 ? (
              ...
              <div className="cart-items-container flex-grow flex flex-col justify-between" style={{ height: '100%' }}>
                <div>
                  {cart.map((item, index) => (
                ...
                <div className="cart-total-section mt-auto pt-4" style={{ borderTop: '1px solid var(--border-color)' }}>
                  <div className="text-right mb-4" style={{ fontSize: '1.25rem', fontWeight: 'bold', color: 'var(--primary-color)' }}>
            ...
            <div className="cart-reservation-form flex flex-col" style={{ height: '100%' }}>
              <button 
              ...
              <form onSubmit={handleReserve} className="flex flex-col gap-4">
                ...
                <button type="submit" className="btn btn-primary mt-4" disabled={loading}>
                  {loading ? 'Réservation en cours…' : '🛍️ Valider ma réservation'}
                </button>
              </form>
            </div>
          )}
